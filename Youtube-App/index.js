...
async function fetchResults() {
  ...
  var response = await fetch(
    `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${searchedValue}&maxResults=20&key=[your_api_key]`
  );
...
async function getVideo(videoId) {
  var response = await fetch(
    `https://www.googleapis.com/youtube/v3/videos?part=snippet,player&id=${videoId}&key=[your_api_key]`
  );
...
async function trendingIndia() {
  var response = await fetch(
    `https://youtube.googleapis.com/youtube/v3/videos?part=snippet&chart=mostPopular&maxResults=40&regionCode=IN&key=[your_api_key]`
  );
